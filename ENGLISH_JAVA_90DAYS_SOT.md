# English + Java 90-Day Program — Single Source of Truth (V11.17 — FROZEN)
...
English Active Topics（已学 Topic 总数达到 7 个，继续处于第 7 节
Priority-Based Review Engine 抽样机制下，非全覆盖；下列 7 个 Topic 按
Learning Recency 排序列出，供下次 Session 抽样参考）：
- Topic: Weather（最新学习）
- Topic: Shopping
- Topic: Food
- Topic: Family
...
Java Active Questions（当前 5 个，仍 ≤ 5，全部纳入；下次再新增 Interview
Question/Scenario 将首次超过 ≤5 阈值，届时切换为第 7 节 Priority-Based
Review Engine 抽样机制，与 English 侧 Session 8 的切换方式一致）：
- What is Java?
- Why do you use Java?
- Scenario 1：线上接口 RT 从 50ms 飙升到 3 秒，作为值班负责人你会如何处理？
- Scenario 2：线上 CPU 突然达到 100%，你会如何处理？
- Scenario 3：线上某个接口突然返回大量 HTTP 500 错误，你会如何处理？

### 13.2 Session Detail

- Session: 9（已完成，等待开始 Session 10）
- English：完整 Review Topic "Shopping"（5/5 正确，稳定复现）与 Topic
  "Food"（5/5 正确，稳定复现，Review 时使用了 Food Question 2 的一个非
  Canonical 变体 "What do you usually eat?"，未据此覆盖第 11 节已存
  Canonical Question，详见第 11 节该条 Review History）。新学 Topic 7:
  Weather，5 组 Question & Standard Answer（含中文翻译/IPA/中文谐音）已按
  English Learning Template 存入第 11 节，Standard Answer 本次已逐字确认，
  非占位状态。完成 Initial Practice、Immediate Reinforcement、Random
  Review、Combined Questions、Today's Final Review，5 题最终均能正确
  回答。纠正两处：(1) "because it's convenient" 与 "because I can go
  outside" 理由句混用（Topic Interference，已纠正，Standard Answer 未被
  修改，详见第 11 节该条 Review History）；(2) 语序错误 "Summer usually is
  very hot." → "Summer is usually very hot."。Topic 标记 Practicing——本
  Session 内的 Immediate Reinforcement/Today's Final Review 完成不等于
  Mastered，仍需未来 Session 持续稳定 Review（第 9 节 Progress Rules）。
  已学 Topic 总数达到 7 个，继续处于 Priority-Based Review Engine 抽样
  机制下（自 Session 8 起，详见 13.1）。
- Java：Interview Topic "Production Incident Scenarios" 新场景 Scenario 3
  ——单个核心接口大量返回 HTTP 500（Error Rate 0%→35%，RT 80ms→约
  300ms，CPU/Memory/Database/Redis/MQ 均正常）。训练过程中出现两次偏差并
  被用户纠正：AI 最初套用通用故障模板（确认影响→恢复业务→查日志→
  RCA），后又无证据猜测空指针/参数异常/数据转换失败等具体根因；最终确认
  核心原则（HTTP 500 是异常结果而非根因 → 缩小范围 → TraceID/日志/异常
  堆栈取证 → 依据证据决定回滚/修复/降级 → 验证恢复 → RCA），并补充学习了
  Trace/TraceID 的含义。Interviewer Intent、Canonical Thinking、Canonical
  Answer 均已存入第 12 节 Scenario 3，Canonical Answer 与用户最终确认版本
  逐字一致，按第 6 节 Java Canonical Answer Rule 锁定。标记 Practicing
  （按 9.1 节标准，尚未经过后续 Session 的 Random Review 验证）。
- Execution Audit（本 Session 审计结论，均判定为 EXECUTION FAILURE — NO
  ARCHITECTURE CHANGE，详见第 14 节 Learning Log Session 9 完整记录）：
  Session Context Grounding、Review 顺序（英语起始重点应为 Shopping/Food）、
  提前进入 Java、Correct Fast-Path、Accurate/No Assumption Feedback、
  Continuous Flow 均出现执行失败，但均已有现行规则覆盖（第 3/6/7 节），
  未发现真正的 Architecture Bug，因此本 Session 未修改第一部分课程规范，
  版本继续保持 V11.17。本 Session 生成的长篇 Today's Learning Summary 不
  符合第 10 节 Output Format，且曾出现虚构 Topic 与错误标记 Weather 为
  Mastered 的情况，均不作为课程事实，已以本次修复后的 SOT 数据为准。
- Next Session：英语复习 Topic 7 (Weather)，并对 Topic 1–7 做随机复习
  （Priority-Based Review Engine 抽样，不保证全覆盖），Weather 稳定后可
  引入 Topic 8；Java 随机复习既有已学 Interview Question/Scenario（含新增
  的 Scenario 3，不得重新改写其 Canonical Answer），继续沿用 Analyze →
  Narrow Scope → Evidence → Action → Verification → RCA 的排查思路。
