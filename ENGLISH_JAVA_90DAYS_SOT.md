...
English Active Topics（截至 Session 12 / 2026-07-16。**本 Session 是 7 天
窗口首次出现内容滚出**：Self Introduction / English Learning（均
2026-07-06，已 10 天）、Daily Routine（2026-07-07，已 9 天）超过 7 天，
按第 7 节说明移出默认高频复习池，改为低频抽查，不代表内容被排除或
删除；下列窗口内内容按 Learning Recency 排序）：
- Topic: Technology（最新学习，Level 1，Practicing，Level 1 首轮练习 5
  轮 Review 已完成，2026-07-16）
- Topic: Travel（Practicing，2026-07-14）
- Topic: Hobbies（2026-07-12）
- Topic: Weather（2026-07-11）
- Topic: Shopping（2026-07-10）
- Topic: Food（2026-07-09，第 7 天边界，仍计入窗口）
- Topic: Family（2026-07-09，第 7 天边界，仍计入窗口）
...
Daily Native Expressions Active Window（V11.19 新增，见第 4.5/11.2 节）：
- Day 1（场景：早晨起床流程，Practicing，3 轮 Review 已完成，2026-07-16）

Java Active Questions（截至 Session 11，本 Session 未安排 Java 练习，快照
维持不变；"JVM Full GC"（Session 10）与 "Database Connection Pool
Exhausted"（Session 11）两个场景仍因缺少逐字中文信息未写入第 12 节，详见
13.2 与 SOT Check。按 7 天窗口计算，"What is Java?"/"Why do you use
Java?"（2026-07-06）已滚出窗口，转入低频抽查池，其余三个场景仍在窗口内，
但 V11.19 起 Java 练习本身已非默认环节，窗口状态仅供下次安排 Java 练习时
参考）：
- Scenario 1：线上接口 RT 从 50ms 飙升到 3 秒，作为值班负责人你会如何处理？（2026-07-09，第 7 天边界，仍计入窗口）
- Scenario 2：线上 CPU 突然达到 100%，你会如何处理？（2026-07-09，第 7 天边界，仍计入窗口）
...
- Session: 12（英语部分已同步；本 Session 未涉及 Java，符合 V11.19 起
  Java 非每日固定环节的新架构；Session 10/11 遗留的两个 Java 场景仍待
  用户补充逐字信息）
- 架构：本 Session 新增 V11.19（Level System + Daily Native Expressions
  模块 + 第 4.2 节六步流程重构 + Java 移出每日固定流程 + 第 7 节轮内去重
  规则，详见文档开头版本历史与第 3/4.1/4.2/4.4/4.5/5/7/8/9.1 节）。用户
  提交 Session 12 学习总结，其中"新永久课程规则"部分提出四项架构级变更，
  审计后发现均非现有规则的简单执行改进，逐项通过 AskUserQuestion 与用户
  确认后采纳，版本由 V11.18 推进至 V11.19，详见文档开头 V11.19 说明。
- English：新学 Topic 10: Technology（Level 1），6 组 Q&A（Do you like
  technology? / What technology do you use every day? / How does
  technology help you at work? / What is your favorite app? / Do you
  think AI is useful? / What new technology would you like to learn?），
  用户会话总结已提供逐字 Standard Answer，中文翻译/IPA/中文谐音由 Claude
  根据标准发音补充（沿用既有做法），已按第 11.1 节模板存入第 11 节，
  Level 字段标注为 1。完成 5 轮 Review，标记 Practicing。按第 4.4 节
  Topic 推进门槛（V11.19），Level 1 完成即满足开始下一个新 Topic 的
  条件——这也解释了本 Session 为何在 Topic 9（Travel）仍为 Practicing 的
  情况下开始 Topic 10（并非执行失误，是 V11.19 架构变更的预期行为）；
  Topic 9 仍需在后续 Session 通过 Review 循环推进到 Level 2/3 并最终达到
  第 4.4 节"完全完成"标准。另完成 Recent 7-Day Random Review（覆盖 Self
  Introduction / English Learning / Daily Routine / Family / Food /
  Shopping / Weather / Hobbies / Travel / Technology 全部 10 个已学
  Topic），整体表现稳定；语法纠正两处（均为一般性纠正，未指明对应具体
  Topic 条目，未回填改写第 11 节任何固定 Standard Answer）："My favorite
  hobby is read books" → "My favorite hobby is reading books"（hobby
  is + doing）；"They helps me relax" → "They help me relax"（第三
  人称单复数一致）。另记录两类易混淆点，供后续 Session 重点复习：(1)
  Shopping "How often do you usually go shopping?" 与 Travel "How
  often do you usually travel?" 两个 Question 句式相近，容易在 Random
  Review 中混用；(2) Shopping 答案 "once a week" 与 Travel 答案 "once
  or twice a year" 内容相近，容易混淆，第 11 节对应 Standard Answer
  未因此纠正或修改，仅在此记录以提示后续复习方向。已学 Topic 总数达到
  10 个。
- Daily Native Expressions：新学 Day 1（场景：早晨起床流程），3 句（I
  wake up at seven o'clock every morning. / I check my phone after I
  wake up. / After that, I take a shower before breakfast.），已按第
  4.1 节模板存入第 11.2 节。完成 3 轮 Review，纠正 after/before 混淆、
  "my phone" 冠词遗漏、"after I wake up" 语序三处错误，用户总结判定为
  Graduated，按第 4.5 节与第 9.1 节跨 Session 稳定要求，本 Session 内
  3 轮达标记为初步达标，SOT 记录为 Practicing，需后续 Session 持续稳定
  表现才升级 Mastered。
...
- Next Session：英语按第 4.2 节六步流程进行（Review 昨天 Topic → Review
  昨天 Daily Native Expressions → 7 天随机复习 → 学 Topic 10 Level 2
  ["为每个 Answer 增加一句话"] → 学 Topic 11 Level 1 → 生成总结）；Daily
  Native Expressions 新学 Day 2；随机复习继续覆盖最近 7 天窗口内容
  （Family / Food 即将于下次 Session 后滚出窗口，需留意）。Java 若用户
  安排练习，优先补充 "JVM Full GC" 与/或 "Database Connection Pool
  Exhausted" 的逐字信息以正式写入第 12 节并锁定。
