...
Generated: 2026-07-11（基于 SOT V11.17，Session 9 完成后状态，新增 Topic 7
Weather；已学 English Topic 数达到 7，继续处于 Priority-Based Review
Engine 抽样机制下；新增 Java Scenario 3 [HTTP 500]）

---

## 1. English Active Review Window

已学 Topic 总数为 7，超过 ≤5 阈值，以下按 Learning Recency 排序（全部为
Practicing，Weather 为最新学习、最高优先级），供抽样参考，不再保证每次
全覆盖：

1. Weather — Practicing，最高优先级，最新学习
2. Shopping — Practicing，高优先级
3. Food — Practicing，高优先级
4. Family — Practicing
5. Daily Routine — Practicing
6. English Learning — Practicing
7. Self Introduction — Practicing（最早学习，仍在正常轮换中）

下一次 Session 必须优先复习 Weather，再按 Mastery Status + Learning
Recency 抽取其他题目。

不得：
- 虚构不存在的 "Good" Topic；
- 恢复到 ≤5 Topic 的全量覆盖模式；
- 把 Weather 标记为 Mastered；
- 把 Family 错误标记为最新 Topic。
...
### Topic: Weather（新增，Session 9，全部 Practicing）
...
5. Q: Is it hot in summer? — A: Yes, it is. Summer is usually very hot.（注意：不得说成 "Summer usually is very hot"，语序有误）
...
- Interview Topic: Production Incident Scenarios
  ...
  - Scenario 3: 线上某个核心接口突然大量返回 HTTP 500，但只有一个接口受
    影响，系统资源和公共依赖均正常，应如何分析和处理？（新增，Session 9）

（当前已学 Question/Scenario 数为 5，仍 ≤ 5，全部纳入窗口；下次再新增
Interview Question/Scenario 将首次超过阈值，届时切换为 Priority-Based
Review Engine 抽样机制）
...
### Scenario 3: 线上某个核心接口突然大量返回 HTTP 500（新增，Session 9）
...
（Mastery Status: Practicing — 首次锁定，尚未经过后续 Session 的 Random
Review；训练过程中曾出现两次偏差——套用通用故障模板、无证据猜测空指针/
参数异常等具体根因——均已被用户纠正，不得作为 Canonical Answer 的一部分；
学员在此过程中学习了 Trace/TraceID 的含义：TraceID 是定位证据的入口，
不是根因本身）
...
- **English Coverage Rule（已切换）**：已学 Topic 数达到 7（> 5），Review
...
- English：优先复习 Topic 7（Weather），再按 Priority-Based Review Engine
  （Mastery Status + Learning Recency）随机抽取 Shopping、Food 及其他旧
  Topic；Weather 表现稳定后可引入 Topic 8；新内容之后必须完成 Immediate
  Reinforcement 与 Today's Final Review 才可进入 Java。
- Java：随机复习既有已学 Interview Question/Scenario（What is Java? / Why
  do you use Java? / Scenario 1 / Scenario 2 / Scenario 3），Scenario 3
  不得重新生成不同逻辑的 Canonical Answer。每 Session 最多新增一道场景题；
  新场景题须先给 Interviewer Intent 和 Canonical Thinking，再给简洁、
  口语化、约 20–30 秒的参考答案，不得无证据猜测根因。
