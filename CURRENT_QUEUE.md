...
## 1. English Active Review Window（V11.19：7 天滚动窗口 + 低频抽查分层
+ 轮内去重）

规则（第 7 节，叠加在 Priority-Based Review Engine 之上，不替换）：日常
复习默认只从最近 7 天内学习的 Topic 中抽取；超过 7 天的 Topic 不退出复习
体系，改由 Priority-Based Review Engine 做低频抽查，不放入默认队列；同一
...
## 1.5 Daily Native Expressions Active Window（V11.19 新增）

- Day 1（场景：早晨起床流程）— Practicing，已完成 3 轮 Review，
  2026-07-16
  1. I wake up at seven o'clock every morning.
  2. I check my phone after I wake up.
  3. After that, I take a shower before breakfast.
...
### Topic: English Learning（已滚出 7 天窗口，低频抽查）
...
### Topic: Daily Routine（已滚出 7 天窗口，低频抽查）
...
5. Q: Do you usually pay by cash or by card? — A: （本 Session 未逐字记录，待补全；注意此题曾误说成 "What do you usually pay by cash or by card?"，为 Session 8 唯一重复出现的错误，仍需留意；答案 "once a week" 与 Travel 的 "once or twice a year" 易混淆）
...
### Topic: Hobbies
1. Q: What is your hobby? — A: My hobby is reading books. I enjoy reading because I can learn new things.（注意发音：hobby 不读 "hoppy"/"habby"；注意句型：My favorite hobby is **reading** books，不是 "is read books"）
...
4. Q: Do you like watching movies? — A: Yes, I do. I like watching movies because **they help** me relax（不是 "they helps"，第三人称单复数一致）.
5. Q: What do you usually do to relax? — A: I usually read books or watch movies to relax.

### Topic: Travel（Standard Answer 待补全）
1. Q: Do you like traveling? — A: （本 Session 未提供逐字标准答案，待补全）
2. Q: Where do you want to travel in the future? — A: （本 Session 未提供逐字标准答案，待补全）
3. Q: Who do you want to travel with? — A: （本 Session 未提供逐字标准答案，待补全）
4. Q: How do you usually travel? — A: （本 Session 未提供逐字标准答案，待补全；Session 12 Random Review 曾使用非 Canonical 变体 "How often do you usually travel?"，与 Shopping "How often do you usually go shopping?" 易混淆，Canonical Question 未被覆盖，仍以本条为准）
...
## 3. Java Active Review Window（V11.18/V11.19：7 天滚动窗口 + 低频抽查
分层；V11.19 起 Java 非每日固定环节，本窗口仅供下次安排 Java 练习时参考）

规则同上方英语部分：日常 Java Random Review 默认只从最近 7 天内引入的
...
（**未包含**以下两个场景——均已由用户报告完成三轮训练并"固化"了 Final
Canonical Answer，但均缺少逐字 Interview Question 中文措辞、Interviewer
Intent 分析与符合既有格式的完整中文 Canonical Answer 正文，按 Java
Canonical Answer Rule 的"不得自行重构正文"先例，暂未写入 SOT 第 12 节，
因此也未列入本窗口；待用户分别补充后才会正式纳入）：
- "JVM Full GC causing high RT"（Session 10 报告，约 98/100）
- "Database Connection Pool Exhausted"（Session 11 报告，100/100）
...
### （待补充）JVM Full GC causing high RT
...
完整中文 Canonical Answer 正文——待用户补充后方可正式锁定。

### （待补充）Database Connection Pool Exhausted

本场景**尚未**正式写入 SOT 第 12 节，仅记录用户 Session 11 报告的英文要点
草稿供参考，不得作为已锁定 Canonical Answer 使用：
Confirm impact → Restore service → Find root cause → Fix the problem →
Verify recovery → RCA。根因方向：Slow SQL / Long transaction /
Connection leak / Connection not released / Connection pool monitoring /
Thread dump / Database sessions。核心原则："Never blindly increase the
connection pool size"（不得盲目扩大连接池大小）。用户报告 Final Score
100/100。**缺少**：逐字 Interview Question 中文措辞、Interviewer Intent
分析、符合 Java Interview Language Rule 与既有格式的完整中文 Canonical
Answer 正文——待用户补充后方可正式锁定。

---

## 5. Session Rules（供外部工具执行时参考的关键约束摘要）

- **六步 Session 流程（V11.19）**：Step1 Review 昨天 Topic → Step2
  Review 昨天 Daily Native Expressions → Step3 最近 7 天随机复习 → Step4
...
- **Curriculum Boundary / Session Context Grounding**：AI 只能从上方
  Active Review Window 中的 Question/Expression/Interview
  Question/Scenario 提问或练习，禁止改写、扩写、临场编造相似或"顺理成章"
  的新问题；只有明确进入 Step4/Step5"学新内容"环节才允许引入新问题。
...
- **Java Canonical Answer Rule**：Canonical Answer 一旦保存即永久锁定，
  仅在缺少逐字确认文本时，不得凭要点/评分自行重构正文写入（"JVM Full GC"
  与 "Database Connection Pool Exhausted" 两个场景均适用此先例，暂缓写入）。
...
- **Scenario Interview Mode**（V11.16）：Java Basics 两个基础问题锁定后，
  此后所有新 Interview Question 默认使用场景型（Scenario）框架，每次
  Java 练习仍只新增一个。
...
- Java：V11.19 起为独立模块，若学员安排练习：优先补充 "JVM Full GC
  ...
  Answer 正文，以正式写入 SOT 第 12 节并锁定；否则随机复习既有已学
  Interview Question/Scenario（What is Java? / Why do you use Java? /
  Scenario 1 / Scenario 2 / Scenario 3），不得重新生成不同逻辑的
  Canonical Answer。
