...
Generated: 2026-07-09（基于 SOT V11.17，Session 7 英语部分完成后状态；
Java Scenario 2「Production CPU 100%」待用户补充 Canonical Answer 原文后
才计入本窗口，见下方第 3/4/6 节说明）
...
### Topic: Food
1. Q: What is your favorite food? — A: （标准答案未逐字记录，练习时以学员当场表达为准）
2. Q: What food do you eat most often? — A: （标准答案未逐字记录，练习时以学员当场表达为准）
3. Q: Can you cook? — A: （标准答案未逐字记录，练习时以学员当场表达为准）
4. Q: What do you usually drink? — A: （标准答案未逐字记录，练习时以学员当场表达为准）
...
- Interview Topic: Production Incident Scenarios
  ...
  - Scenario 2: Production CPU 突然达到 100%（**尚未锁定**——Canonical
    Answer/Interviewer Intent 原文待用户提供，见第 4 节）

（当前已学 Question/Scenario 数为 3（已锁定）+ 1（待锁定）= 4，≤ 5，全部
纳入窗口；Scenario 2 在锁定前不得作为 Random Review 的固定参照标准）
...
### Scenario 2: Production CPU 突然达到 100%（尚未锁定，待补全）

Interviewer Intent、Canonical Thinking 细节、Canonical Answer 原文均待
用户提供逐字文本后写入 SOT 第 12 节，本文件才会同步生成完整内容。已确认
的部分：

**Canonical Thinking（已确认）**：
Impact → Business Recovery → Root Cause Investigation → Recovery
Verification → RCA

三轮得分：99 / 99 / 95。Final Improvement 笔记：Recovery Verification
阶段应包含"先修复问题、再观察系统状态"；补充讲解 Error Rate 概念；面试
第一轮回答应优先"确认业务已恢复"，具体指标（CPU/RT/Error Rate）留待
追问时展开。

**在补全 Canonical Answer 原文之前，Random Review 不得将本 Scenario 当作
已锁定题目抽查，也不得凭空生成替代答案。**
...
- **Course Improvement Validation（非正式规则，仅供参考）**：一种更口语化
  ...
  目前仅在 CPU 场景验证，**尚未采纳进入 SOT**，不得当作正式规则执行。
...
- Java：按 Scenario Interview Mode 引入下一个新场景——"Production OOM
  ...
  仍为 Validation Only，非正式 SOT 规则，见第 5 节）。同时 Scenario 2
  （CPU 100%）的 Canonical Answer 原文一旦提供，应尽快补录入 SOT 第 12
  节并同步本文件。
